...
then
    ...
    IMAGE_TAG_BASE=${DOCKERHUB_ACCOUNT}/ngc_${PLATFORM}_daa:${NGC_VERSION}_${DAA_TAG_BASE}
else
    echo "${ECHO_PREFIX} Base image tag is overridden by the -b option to ${IMAGE_TAG_BASE}. "
fi
IMAGE_TAG_TARGET=${DOCKERHUB_ACCOUNT}/ngc_${PLATFORM}_daa:${NGC_VERSION}_${DAA_TAG_TARGET}
